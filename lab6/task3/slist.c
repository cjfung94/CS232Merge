...
void slist_delete_at(slist_t *l, int index)
{
  snode_t * temp = l->front;
  snode_t * pred = l->front;
  snode_t * end = l->front;

  while (snode_get_next(end) != NULL)
  {
    end = snode_get_next(end); // Gets the end of the list
  }


 if (index < 0)
  {
    if (slist_length(l) + index == 0)
      {
      ...
      {
        temp = slist_find_at(l, slist_length(l) + index) ;
        pred = slist_find_at(l, index - 1); 
        snode_set_next(pred, snode_get_next(temp));
       snode_destroy(temp);
      }
  }
